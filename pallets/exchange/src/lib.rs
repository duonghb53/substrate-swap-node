...
pub mod pallet {
    ...
    // #[pallet::storage]
    // #[pallet::getter(fn balances_account)]
    // pub type BalanceOfAccount<T: Config> = StorageMap<
    //     _,
    //     Twox64Concat,
    //     T::AccountId,
    //     StorageMap<_, Twox64Concat, CurrencyIdOf<T>, OptionQuery>,
    //     ValueQuery,
    // >;
    ...
    pub enum Event<T: Config> {
        ...
        GetFreeBalance(T::AccountId, BalanceOf<T>),
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        #[pallet::weight(10_000)]
        pub fn get_balance_token(
            origin: OriginFor<T>,
            currency_id: CurrencyIdOf<T>,
        ) -> DispatchResultWithPostInfo {
            let who = ensure_signed(origin)?;
            let balance = T::Currency::free_balance(currency_id, &who);
            Self::deposit_event(Event::GetFreeBalance(who, balance));
            Ok(().into())
        }
    }
}
